...
namespace DaySim.ChoiceModels.H.Models {
  public class AggregateTourModeDestinationModel : ChoiceModel {
    ...
    private void RunModel(ChoiceProbabilityCalculator choiceProbabilityCalculator, ITourWrapper tour, IHouseholdDayWrapper householdDay, int sampleSize, IParcelWrapper choice = null) {
      ...
      int segment = Global.Kernel.Get<SamplingWeightsSettingsFactory>().SamplingWeightsSettings.GetTourDestinationSegment(tour.DestinationPurpose, tour.IsHomeBasedTour ? Global.Settings.TourPriorities.HomeBasedTour : Global.Settings.TourPriorities.WorkBasedTour, Global.Settings.Modes.Sov, person.PersonType);

      DestinationSampler destinationSampler = new DestinationSampler(choiceProbabilityCalculator, segment, sampleSize, tour.OriginParcel, choice);
      ...
      foreach (KeyValuePair<DestinationSampler.TourSampleItem, int> sampleItem in sampleItems) {
        ...
        for (int modeIndex = 0; modeIndex <= 3; modeIndex++) {
          dynamic pathTypeModel = modeIndex == 0 ? pathTypeModels.First(x => x.Mode == Global.Settings.Modes.Walk)
          ...
          dynamic modeAvailable = pathTypeModel.Available;
          ...
          dynamic alternative = choiceProbabilityCalculator.GetAlternative(index++, modeAvailable, chosen);
