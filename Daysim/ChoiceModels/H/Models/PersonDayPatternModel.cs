...
namespace DaySim.ChoiceModels.H.Models {
  public class PersonDayPatternModel : ChoiceModel {
    ...
    public void Run(IPersonDayWrapper personDay, IHouseholdDayWrapper householdDay) {
      ...
      ChoiceProbabilityCalculator choiceProbabilityCalculator = _helpers[ParallelUtility.GetBatchFromThreadId()].GetChoiceProbabilityCalculator(personDay.Person.Id * 10 + personDay.Day);
    ...
    private void InitializeDayPatterns(IPersonDayWrapper personDay, DayPattern[] dayPatterns) {
      ...
      int priorMealTours = personDay.MealJointTours > 0 ? 1 : 0;
      int priorSocialTours = personDay.SocialJointTours > 0 ? 1 : 0;
      int priorRecreationTours = personDay.RecreationJointTours > 0 ? 1 : 0;
      int priorMedicalTours = personDay.MedicalJointTours > 0 ? 1 : 0;

      int alternativeIndex = -1;

      for (int recreationTours = 0; recreationTours <= 1; recreationTours++) {
        for (int medicalTours = 0; medicalTours <= 1; medicalTours++) {
              ...
              for (int shoppingTours = 0; shoppingTours <= 1; shoppingTours++) {
                for (int mealTours = 0; mealTours <= 1; mealTours++) {
                  for (int socialTours = 0; socialTours <= 1; socialTours++) {
                    for (int recreationStops = 0; recreationStops <= 1; recreationStops++) {
                      for (int medicalStops = 0; medicalStops <= 1; medicalStops++) {
                            ...
                            for (int shoppingStops = 0; shoppingStops <= 1; shoppingStops++) {
                              for (int mealStops = 0; mealStops <= 1; mealStops++) {
                                for (int socialStops = 0; socialStops <= 1; socialStops++) {
                                  int totalNonMandatoryTourPurposes = recreationTours + medicalTours + escortTours + personalBusinessTours + shoppingTours + mealTours + socialTours;
                                  int totalNonMandatoryStopPurposes = recreationStops + medicalStops + escortStops + personalBusinessStops + shoppingStops + mealStops + socialStops;
                                  int totalTourPurposes = totalNonMandatoryTourPurposes + workTours + schoolTours;
                                  int totalStopPurposes = totalNonMandatoryStopPurposes + workStops + schoolStops;
                                  int totalDayPurposes = Math.Min(workTours, workStops) + Math.Min(schoolTours, schoolStops) + Math.Min(escortTours, escortStops) + Math.Min(personalBusinessTours, personalBusinessStops) + Math.Min(shoppingTours, shoppingStops) + Math.Min(mealTours, mealStops) + Math.Min(socialTours, socialStops) + Math.Min(recreationTours, recreationStops) + Math.Min(medicalTours, medicalStops);
    ...
    private void RunModel(ChoiceProbabilityCalculator choiceProbabilityCalculator, IPersonDayWrapper personDay, IHouseholdDayWrapper householdDay, DayPattern[] dayPatterns, DayPattern choice = null) {
      ...
      int usualModeNotSOVFlag = (person.IsWorker && person.UsualModeToWork != Global.Settings.Modes.Sov).ToFlag();
      int jointToursFlag = personDay.JointTours > 0 ? 1 : 0;
      int proxyCompletedDiaryFlag = person.ProxyResponse > 0 ? 1 : 0;

      double[] purposeLogsums = new double[Global.Settings.Purposes.TotalPurposes + 10];
      ...
      if (person.UsualWorkParcel == null || person.UsualWorkParcelId == household.ResidenceParcelId) {
        ...
        ChoiceProbabilityCalculator.Alternative nestedAlternative = Global.ChoiceModelSession.Get<WorkTourModeTimeModel>().RunNested(personDay, residenceParcel, person.UsualWorkParcel, destinationArrivalTime, destinationDepartureTime, household.VehiclesAvailable);
      ...
      for (int xPurpose = Global.Settings.Purposes.Escort; xPurpose <= Global.Settings.Purposes.Medical + 10; xPurpose++) {
        ...
        int purpose = xPurpose <= Global.Settings.Purposes.Medical ? xPurpose :
        ...
        double xMultiplier = xPurpose <= Global.Settings.Purposes.Medical ? 1.0 :
      ...
      for (int alternativeIndex = 0; alternativeIndex < TOTAL_ALTERNATIVES; alternativeIndex++) {
        ...
        for (int purpose = Global.Settings.Purposes.Escort; purpose <= Global.Settings.Purposes.Medical; purpose++) {
        ...
        for (int tourPurpose = Global.Settings.Purposes.Work; tourPurpose <= Global.Settings.Purposes.Medical; tourPurpose++) {
          for (int stopPurpose = Global.Settings.Purposes.Work; stopPurpose <= Global.Settings.Purposes.Medical - 1; stopPurpose++) {
        ...
        for (int tourPurpose = Global.Settings.Purposes.Work; tourPurpose <= Global.Settings.Purposes.Medical - 2; tourPurpose++) {
          for (int tourPurpose2 = tourPurpose + 1; tourPurpose2 <= Global.Settings.Purposes.Medical; tourPurpose2++) {
        ...
        for (int stopPurpose = Global.Settings.Purposes.Work; stopPurpose <= Global.Settings.Purposes.Medical - 2; stopPurpose++) {
          for (int stopPurpose2 = stopPurpose + 1; stopPurpose2 <= Global.Settings.Purposes.Medical; stopPurpose2++) {
    ...
    private sealed class DayPattern {
      ...
      public bool Equals(DayPattern other) {
        ...
        int recreationToursFlag = (RecreationTours > 0).ToFlag();
        int medicalToursFlag = (MedicalTours > 0).ToFlag();
        ...
        int recreationTours2Flag = (other.RecreationTours > 0).ToFlag();
        int medicalTours2Flag = (other.MedicalTours > 0).ToFlag();
        ...
        int recreationStopsFlag = (RecreationStops > 0).ToFlag();
        int medicalStopsFlag = (MedicalStops > 0).ToFlag();
        ...
        int recreationStops2Flag = (other.RecreationStops > 0).ToFlag();
        int medicalStops2Flag = (other.MedicalStops > 0).ToFlag();
