...
namespace DaySim.ChoiceModels.H.Models {
  public class OtherHomeBasedTourModeTimeModel : ChoiceModel {
    private const string CHOICE_MODEL_NAME = "HOtherHomeBasedTourModeTimeModel";
    ...
    public override void RunInitialize(ICoefficientsReader reader = null) {
      Initialize(CHOICE_MODEL_NAME, Global.Configuration.OtherHomeBasedTourModeTimeModelCoefficients, HTourModeTime.TotalTourModeTimes, TOTAL_NESTED_ALTERNATIVES, TOTAL_LEVELS, MAX_PARAMETER);
    }
    ...
    //MBADD runnested
    public ChoiceProbabilityCalculator.Alternative RunNested(IPersonWrapper person, IParcelWrapper originParcel, IParcelWrapper destinationParcel, int destinationArrivalTime, int destinationDepartureTime, int householdCars, int transitDiscountFraction) {
      if (person == null) {
    ...
    private void RunModel(ChoiceProbabilityCalculator choiceProbabilityCalculator, IHouseholdDayWrapper householdDay, ITourWrapper tour,
      ...
      {
        foreach (HTourModeTime modeTimes in HTourModeTime.ModeTimes[ParallelUtility.GetBatchFromThreadId()]) {
