...
namespace DaySim.ChoiceModels.H.Models {
  public class WorkUsualModeAndScheduleModel : ChoiceModel {
    ...
    private void RunModel(ChoiceProbabilityCalculator choiceProbabilityCalculator, IPersonWrapper person) {
      ...
      IHouseholdWrapper household = person.Household;
      ...
      IParcelWrapper originParcel = household.ResidenceParcel;
      IParcelWrapper destinationParcel = person.UsualWorkParcel;
      ...
      double destinationParkingCost = (Global.Configuration.ShouldRunPayToParkAtWorkplaceModel && person.PaidParkingAtWorkplace == 0) ? 0.0 : destinationParcel.ParkingCostBuffer1(parkingDuration);

      double income = household.Income < 0 ? Global.Configuration.Coefficients_BaseCostCoefficientIncomeLevel : household.Income; // missing converted to 30K
