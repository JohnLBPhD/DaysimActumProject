  ...
  public class TripModeModel : ChoiceModel {
    ...
    public void Run(IHouseholdDayWrapper householdDay, ITripWrapper trip) {
      ...
      if (_helpers[ParallelUtility.GetBatchFromThreadId()].ModelIsInEstimationMode) {
        ...
        dynamic pathTypeModel = pathTypeModels.First(x => x.Mode == mode);
    ...
    private void RunModel(ChoiceProbabilityCalculator choiceProbabilityCalculator, ITripWrapper trip, IEnumerable<dynamic> pathTypeModels, IParcelWrapper originParcel, IParcelWrapper destinationParcel,
      ...
      dynamic pathTypeExtra = pathTypeModels.First(x => x.Mode == Global.Settings.Modes.Hov3);
      int modeExtra = Global.Settings.Modes.SchoolBus;
      dynamic availableExtra = pathTypeExtra.Available && tour.IsSchoolBusMode() && tripModeAvailable[modeExtra]
           && (trip.IsHalfTourFromOrigin
      ...
      dynamic generalizedTimeLogsumExtra = pathTypeExtra.GeneralizedTimeLogsum;

      dynamic alternative = choiceProbabilityCalculator.GetAlternative(modeExtra, availableExtra, choice == modeExtra);
