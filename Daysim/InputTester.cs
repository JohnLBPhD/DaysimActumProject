...
namespace DaySim {
  public static class InputTester {
    ...
    public static void RunTest() {
      errorCount = 0;
      Dictionary<int, int> zones = GetValidZones();
      Dictionary<int, int> parcels = TestParcels(zones);

      if (Global.Configuration.ReadHDF5) {
        Dictionary<int, int> households = TestHouseholds(parcels);
    ...
    private static Dictionary<int, int> GetValidZones() {
      ...
      using (StreamReader reader = new StreamReader(inputFile.Open(FileMode.Open, FileAccess.Read, FileShare.Read))) {
        int newId = 0;
        string line;
        int lineNum = 1;
    ...
    private static Dictionary<int, int> TestParcels(Dictionary<int, int> zones) {
      Dictionary<int, int> parcels = new Dictionary<int, int>();
      FileInfo inputFile = Global.GetInputPath(Global.Configuration.RawParcelPath).ToFile();
    ...
    private static Dictionary<int, int> TestHouseholds(Dictionary<int, int> parcels) {
      Dictionary<int, int> households = new Dictionary<int, int>();
      //has hhno and hhsize
          ...
          switch (vector) {
            ...
            case "hhparcel":
              if (values[x].Except(parcels.Keys).Count() > 0) {
                IEnumerable<int> badPcls = values[x].Except(parcels.Keys);
        ...
        switch (vector) {
          case "hhno":
            ...
            if (values[x].Except(households.Keys).Count() > 0) {
              IEnumerable<int> badPeople = values[x].Except(households.Keys);
    ...
    private static void TestParkAndRideNodes(Dictionary<int, int> zones) {
      ...
      try {
        using (StreamReader reader = new StreamReader(rawFile.Open(FileMode.Open, FileAccess.Read, FileShare.Read))) {
          reader.ReadLine();
