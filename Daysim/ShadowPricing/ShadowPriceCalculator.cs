...
namespace DaySim.ShadowPricing {
  public static class ShadowPriceCalculator {
    public static void CalculateAndWriteShadowPrices() {
      ...
      Dictionary<int, ShadowPriceZone> zoneShadowPrices = new Dictionary<int, ShadowPriceZone>();

      foreach (Framework.DomainModels.Wrappers.IParcelWrapper parcel in ChoiceModelFactory.Parcels.Values) {
        if (!zoneShadowPrices.ContainsKey(parcel.ZoneId)) {
          zoneShadowPrices.Add(parcel.ZoneId, new ShadowPriceZone());
        }

        ShadowPriceZone shadowPriceZone = zoneShadowPrices[parcel.ZoneId];
      ...
      double employmentFactor = employmentPrediction / Math.Max(externalEmploymentTotal, 1);
      double studentsK12Factor = studentsK12Prediction / Math.Max(externalStudentsK12, 1);
      double studentsUniversityFactor = studentsUniversityPrediction / Math.Max(externalStudentsUniversity, 1);

      using (ShadowPriceWriter shadowPriceWriter = new ShadowPriceWriter(new FileInfo(Global.ShadowPricesPath))) {
        foreach (Framework.DomainModels.Wrappers.IParcelWrapper shadowPriceParcel in ChoiceModelFactory.Parcels.Values) {
