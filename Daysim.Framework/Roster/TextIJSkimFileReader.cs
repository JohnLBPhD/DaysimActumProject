...
namespace DaySim.Framework.Roster {
  public sealed class TextIJSkimFileReader : ISkimFileReader {
    ...
    public SkimMatrix Read(string filename, int field, float scale) {
      ...
      if (!_cache.TryGetValue(file.Name, out List<double[]> rows)) {
        //				rows = new List<float[]>();
        ...
        using (StreamReader reader = new StreamReader(file.Open(FileMode.Open, FileAccess.Read, FileShare.Read))) {
          ...
          while ((line = reader.ReadLine()) != null) {
            ...
            try {
              ...
              string[] p1 = line.Split(new[] { delimiter }, StringSplitOptions.RemoveEmptyEntries);
              //							var row = p1.Select(Convert.ToSingle).ToArray();
              double[] row = p1.Select(Convert.ToDouble).ToArray();     // 20150703 JLB
      ...
      for (int i = 0; i < rows.Count; i++) {
        ...
        try {
          ...
          // test code
          int index = Convert.ToInt32(row[0]);
          int origin = _mapping[index];
          //var origin = _mapping[Convert.ToInt32(row[0])];

          index = Convert.ToInt32(row[1]);
          int destination = _mapping[index];
          //var destination = _mapping[Convert.ToInt32(row[1])];

          float rawValue = Convert.ToSingle(row[field + FIELD_OFFSET]);
          ...
          ushort value = Convert.ToUInt16(rawValue * scale);
