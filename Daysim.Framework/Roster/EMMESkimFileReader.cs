...
namespace DaySim.Framework.Roster {
  public class EMMESkimFileReader : ISkimFileReader {
    ...
    public SkimMatrix Read(string filename, int field, float scale) {
      GetEMMEMatrix(filename, field, scale);
    ...
    private void GetEMMEMatrix(string filename, int matrixNumber, float scale) {
      ...
      ReadEMMEBankHeader();
      long curptr = _ptr64 + _mcent * _mcent * (matrixNumber - 1);
    ...
    private void ReadEMMEBankHeader() {
      ...
      uint ptr1 = _reader.ReadUInt32();
      ptr1 = (ptr1 & mask);
      ptr1 *= 4;
      ptr1 += 52 * 4;
      _reader.BaseStream.Seek(ptr1 - 4, SeekOrigin.Begin);
      ...
      uint ptr60 = _reader.ReadUInt32() & mask;
