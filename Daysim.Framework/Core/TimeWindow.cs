...
namespace DaySim.Framework.Core {
  public sealed class TimeWindow : ITimeWindow {
    ...
    public void SetBusyMinutes(int inclusiveStart, int exclusiveEnd) {
      int duration = Math.Abs(exclusiveEnd - inclusiveStart);
      ...
      int inclusiveEnd = inclusiveStart + duration - 1;
    ...
    private void MergeBusyMinutes(int start, int end) {
      ...
      bool containedInExistingRange = false;

      foreach (IMinuteSpan busyMinute in _busyMinutes) {
    ...
    private List<IMinuteSpan> GetAvailableMinutes() {
      List<IMinuteSpan> availableMinutes = new List<IMinuteSpan>();
    ...
    //     duration of at least timeTo + mad + timeFrom  (used in HTourModeTime)
    {
      ...
      int maxDuration = 0;
      foreach (IMinuteSpan availableMinute in _availableMinutes.Where(availableMinute =>
    ...
    public int TotalAvailableMinutesBefore(int minute) {
      ...
      foreach (
          IMinuteSpan availableMinute in
    ...
    public int GetAvailableMinute(IRandomUtility randomUtility, int inclusiveStart, int exclusiveEnd,
      ...
      foreach (IMinuteSpan availableMinute in _availableMinutes) {
    ...
    public ITimeWindow DeepCloneToANewWindow() {
      ...
      foreach (IMinuteSpan busySpan in _busyMinutes) {
        MinuteSpan newBusySpan = new MinuteSpan(busySpan.Start, busySpan.End);
        newTimeWindow._busyMinutes.Add(newBusySpan);
      }
      foreach (IMinuteSpan availableSpan in _availableMinutes) {
        MinuteSpan newAvailableSpan = new MinuteSpan(availableSpan.Start, availableSpan.End);
    ...
    public bool ValidTimeWindow() {
      ...
      foreach (IMinuteSpan span in _availableMinutes) {
      ...
      foreach (IMinuteSpan bSpan in _busyMinutes) {
        foreach (IMinuteSpan aSpan in _availableMinutes) {
